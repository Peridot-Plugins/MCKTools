                ...
                if (p.hasPermission("mckt.tpa")) {
                    if (args.length >= 1) {
                        ...
                        if (Bukkit.getPlayer(pName) != null) {
                            ...
                            u.setTpaPlayer(p);
                            u.setTpaType(true);
                            p.sendMessage(ChatUtil.color(FileManager.getMsg().getString("messages.tpa.msgyou").replace("{PLAYER}", pa.getName())));
                            for (String msgs : FileManager.getMsg().getStringList("messages.tpa.msgarg")) {
                        ...
                        p.sendMessage(ChatUtil.color(FileManager.getMsg().getString("messages.globalerrors.correctusage")).replace("{CORRECTUSAGE}", "/tpa [gracz]"));
                    }
                } else {
                    p.sendMessage(ChatUtil.color(FileManager.getMsg().getString("messages.globalerrors.noperm")).replace("{PERM}", "mckt.tpa"));
                }
            } else {
